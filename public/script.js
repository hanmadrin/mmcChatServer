...
const functions = {
    mondayFetch: async (query,files=null,version='2026-01') => {
...
const pages = {
    ...
    home: async () => {
        ...
        const aiUsers = globals.aiUsers;
        if (aiUsers.includes(userName)) {
            const aiLinksData = {
                aiAutomation: 'AI Automation',
            }
            for (const link in aiLinksData) {
                const linkElement = document.createElement('a');
                linkElement.href = '/' + link;
                linkElement.classList = 'btn box-shadow-inset d-block p-20px border-radius-5px text-white link-decoration-none';
                linkElement.innerText = aiLinksData[link];
                main.append(linkElement);
            }
        }
        controllers.popup({ state: false });
...
const view = async () => {
    ...
    if (path == '/') {
        await pages.home();
    } else if ((path == '/aiAutomation' || path == '/aiAutomation/') && aiUsers.includes(userName)) {
        // await aiAutomationPage();
        controllers.popup({ state: false });
